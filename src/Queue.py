...
class QueueMode:
  ...
  @staticmethod
  def IsValid(qType):
    if ( qType == QueueMode.QUEUE or 
         qType == QueueMode.MUTEX or 
         qType == QueueMode.COUNTING or 
         qType == QueueMode.BINARY or 
         qType == QueueMode.RECURSIVE ):
      return(True)
    else:
      return(False)
  ...
  def GetQueueType(self): 
    ...
    try: 
      ...
      if ( QueueMode.IsValid(int(qType)) ):
        ...
        raise ValueError("Invalid Queue Type In Queue Object! Are you sure this is a Queue Handle?")
